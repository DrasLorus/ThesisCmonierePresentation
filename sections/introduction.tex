...
\begin{frame}{Internet des objets}
  ...
  \note{
    \begin{itemize}
      \item Un "objet connecté" c'est un dispositifs basse conso, bas-coût, spécialisé (thermomètres, capteur de lumière, domotique, etc) -> noeud-capteurs
      \item parle pas souvent, quand il parle on veut l'entendre
      \item Chez les autres, on cherche le débit ++, moins dans l'IoT
    \end{itemize}
  }
\end{frame}
...
\begin{frame}{Réseaux longues portées basses puissances}
  ...
  \note{
    \begin{itemize}
      ...
      \item MAIS déploiement massif DONC bruit
      \item paquets courts
      \item basse conso'
...
\begin{frame}{\acrshortpl{lpwan} : trois exemples}
  ...
  \note {
    bla bla bla \vspace{1 em}

    contexte : LoRa donc $50$ kb/s en prio., portée secondaire.
  }
\end{frame}
...
\begin{frame}{Chaine de communication classique}{} \centering \vspace{-1em}
  ...
  \end{columns}
  \note{
    Décrit la chaine puis vite :\\
    \begin{itemize}
      \item Message binaire
      \item codage / redondance pour robustesse
      \item Metadonnée en préambule pour simplifier détection et synchro
      \item Canal (ex radio)
      \item Detec/sync grace à rouge
      \item Decodage
      \item message
    \end{itemize}
    Cadre thèse ->\\

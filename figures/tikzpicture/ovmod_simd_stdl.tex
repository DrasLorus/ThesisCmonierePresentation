...
\begin{tikzpicture} [-latex,
  ...
  \node[anchor = west, align = center] at (symb3.east)  {Si $B(i) = 1$,};
  \node[anchor = west, align = center] at (symbb3.east) {Si $B(i) = 0$.};

  \node[anchor = south, align = center] at (ccsk1.north east) {$i^{\textrm{th}}$ Symbole CCSK\\($q$ chips)};
  \node[anchor = north, align = center] at (symbb2.south west) {$i^{\textrm{th}}$ Symbole QCSP\\($q$ chips)};

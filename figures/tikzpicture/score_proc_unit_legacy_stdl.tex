...
\begin{tikzpicture} [->,
  ...
  \node [draw = none, anchor = west, align = center] at (ankdsh.north east) {Tous les $\frac{q}{p_\Delta}$\\chips};
  ...
  \node [main node,
    ...
    % fill = gray!40!white, 
    right = .8cm of corr
  ] (mabs) {Maximum\\valeur absolue};

  \draw [line width = 2.6pt] (corr.east) -- (mabs.west)
  node [midway, above = .1 cm, align=center] () {$\vect{L}^\omega_n$};
  ...
  \node [right = 3.2 cm of add] (out) {$S^\omega_n$};

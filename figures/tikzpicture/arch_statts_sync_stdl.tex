...
\begin{tikzpicture} [->,
  ...
  \node [
    ...
    below  = .5 of inrot,
    font   = {\Large},
  ] (ejw) {$e^{-j\frac{\omega n}{q}}$};
  \draw[] (ejw.north) -- (inrot.south);

  \node [anchor = west] (lnw) at ($(yn) + (15.1, 0)$) {$\vect{L}_n^\omega$};
  ...
  \node [anchor = north west] at ($(add0 -| reg0) + (.1, 0)$) {$l_n^\omega(i)$};
  \node [anchor = north west] at ($(add1 -| reg1) + (.1, 0)$) {$l_n^\omega(q - 1)$};
  \node [anchor = north west] at ($(add2 -| reg2) + (.1, 0)$) {$l_n^\omega(0)$};

  \node [anchor = south] at ($(add0 |- reg0.west) + (-.25, 0)$) {$l_{n - 1}^\omega(i - 1)$};
  \node [anchor = south] at ($(add1 |- reg1.west) + (-.25, 0)$) {$l_{n - 1}^\omega(q - 2)$};
  \node [anchor = south] at ($(add2 |- reg2.west) + (-.25, 0)$) {$l_{n - 1}^\omega(q - 1)$};

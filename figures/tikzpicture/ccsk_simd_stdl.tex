...
\begin{tikzpicture} [-latex,
  ...
  \node [main node,
    ...
    anchor=north east,
    align = center
  ] (in) at ($(ccsk0.south west) + (-1,-1)$) {Entrée\\($p$ bits)};
  ...
  \node[anchor = south, align = center] at (ccsk1.north east) {Séquence CCSK \pn{0}\\($q$ chips)};
  \node[anchor = north, align = center] at (symb1.south east) {Symbole CCSK\\($q$ chips)};
